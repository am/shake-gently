...
- **Shared document** -- open the app in several browser tabs (or machines) and type. Every keystroke syncs instantly through the Cloudflare Durable Object backend (or Miniflare locally via `wrangler dev`).
...
| [ws](https://github.com/websockets/ws)                        | Node WebSocket server (test helper only)                 |
...
npm run start     # wrangler dev (background) + Vite dev server
# or separately:
npm run server    # just wrangler dev (WS relay on :8787)
npm run dev       # just Vite

npm run lint            # Biome linter across the whole project
npm test                # Node integration test (needs wrangler dev on :8787)
...
Open `http://localhost:5173` in multiple tabs to collaborate. Vite serves the frontend with HMR; `wrangler dev` runs the real Worker + Durable Object locally via Miniflare on port 8787. The client connects to `ws://localhost:8787` in dev mode; on any other origin it derives the WebSocket URL from the page host automatically, so no build-time config is needed for Cloudflare deployments. Set `VITE_WS_URL` to override this if needed.
